...
branches = tree.arrays()[['sigJetRecur_dr12',
                          'sigJetRecur_jetpt',
                          'sigJetRecur_z']]  # 'branches' now stores the observables in 'awkward arrays'

# content of first 10 events
print("\nFirst 10 events:", *
      (entry for entry in branches['sigJetRecur_dr12'][:10]), sep="\n")

# jets of the first event
print("\nJets of first event:", *
      (jet for jet in branches['sigJetRecur_dr12'][0]), sep="\n")

# they work like numpy arrays and you can use numpy functions on them
print("\nnp.exp(jet)", *(np.exp(jet)
      for jet in branches['sigJetRecur_dr12'][0]), sep="\n")
...
print("type of a numpy converted jet:", type(
    ak.to_numpy(branches['sigJetRecur_dr12'][0, 1])))
